...
* Use the FPGA in a sensible manner. The resulting implementation can achieve multiplier utilization close to 100 %, that's 1.9e9 multiplications per second (on a USB bus power budget of ~2 Watts)
...
* Realistic CPU size: For a CPU-centric industrial (not "fun") design I see no alternative to the vendor's proprietary CPU offerings e.g. Microblaze or Zynq ARM. However, for a minimal controller I expect that it is small even if it makes no real difference on the large Xilinx FPGA - I may want to use the same technology on lower-tier FPGAs e.g. Lattice, and then CPU size and efficiency becomes more critical. James Bowman's J1B fits the bill for me, and its 32 bit extension is put to good use in this project.
* Reaching the conclusion (personal opinion!) that J1B gforth would never be acceptable for an industrial project, I started my own simple compiler / assembler "forthytwo.exe". This may seem over the top but the usefulness of simple CPUs like J1B is unquestioned - Possibly with compilation done in an Excel spreadsheet (industrial ASIC reality).
